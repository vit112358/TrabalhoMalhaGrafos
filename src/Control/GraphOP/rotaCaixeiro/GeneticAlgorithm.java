...
public class GeneticAlgorithm {
    ...
    private static Estrutura rotasAer;

    public GeneticAlgorithm(Estrutura rotas) {
        this.rotasAer = rotas;
    }

    //Evolui uma população para a geração seguinte
    public static Populacao evolvePopulation(Populacao pop) {
        Populacao newPopulation = new Populacao(pop.populationSize(), false, rotasAer);
    ...
    private static void mutate(Rota tour) {
        ...
        for(int tourPos1=0; tourPos1 < tour.tourSize(); tourPos1++){
            if(Math.random() < _mutationRate){
                ...
                Aeroporto city1 = tour.getCity(tourPos1);
                Aeroporto city2 = tour.getCity(tourPos2);
    ...
    private static Rota tournamentSelection(Populacao pop) {
        ...
        Populacao tournament = new Populacao(_tournamentSize, false, rotasAer);
